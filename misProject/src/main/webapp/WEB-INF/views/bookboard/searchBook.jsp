			...
			<div class="row mt">
				<div class="col-lg-12">
					...
					<div class="form-panel">
						<h4 class="mb">
							<i class="fa fa-angle-right"></i>검색된 항목 리스트
						</h4>
								...
								<table class="table table-hover">
									...
									<tbody>
										<c:if test="${!empty searchBookList}">
											<c:forEach items="${searchBookList}" var="BookVO"
												varStatus="listStat">
												<tr>
													...
													<td><a href="#"
														onclick="sendData('${BookVO.bookNum}', '${BookVO.bookTitle}','${BookVO.writer }','${BookVO.publisher}')">${BookVO.bookTitle}</a></td>
													<td>${BookVO.writer }</td>
...
<script>
	// 부모창으로 정보 보내기
	function sendData(bookNum, bookTitle, writer, publisher) {
		var bookNum = bookNum;
		...
		if (bookNum == 0) {
			$("#bookNum", opener.document).attr('readonly', false);
			$("#bookNum", opener.document).val('');
			opener.document.frm.bookNum.value = bookNum;

			opener.document.frm.bookTitle.value = bookTitle;
			opener.document.frm.writer.value = writer;
			opener.document.frm.publisher.value = publisher;


		} else if (bookNum > 0) {
			$("#bookNum", opener.document).attr('readonly', true);
			opener.document.frm.bookNum.value = bookNum;
	...
	$('#searchBtn').on(
			...
			function(event) {
				...
				self.location = "searchBook" + '${paging.makeQuery(1)}'
