	...
	<section class="wrapper">
		<div class="row">
          <div class="col-md-12">
			...
			<!-- /col-md-12 -->
						<h3>
							<i class="fa fa-bullhorn"></i>문의 게시판
						</h3>
						<div class='box-body'>
							...
							<c:choose>
								...
								<button id='newBtn button' type="button" class="btn btn-primary"
									onclick="location.href='/member/login'">문의글 작성</button>
								</c:when>
								<c:when test="${!empty member}">
									<button id='newBtn' class="btn btn-round btn-info">
										문의글 작성</button>
								</c:when>
							</c:choose>
						...
						<table class="table table-hover">
							...
							<c:forEach items="${list}" var="QnAVO" varStatus="var">
								<tr>
									...
									<td><a
								href='/qna/readPage${paging.makeSearch(paging.cri.page) }&qNum=${QnAVO.qNum}'>
									${QnAVO.title} </a></td>
							<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm"
									value="${QnAVO.inDate}" /></td>
							<td>${QnAVO.memName}</td> 
							<td>
							${QnAVO.commCnt}</td>
								</tr>
